...
/**
 * Debt & Credit Agent — watches loans and rates. It is deliberately set to
...
export const debtAgent: Agent = {
  meta: { id: 'debt', name: 'Debt & Credit Agent', blurb: 'Watches your loans and rates', emoji: '🏦', accent: '#8A6FE8' },
  ...
  evaluate(event) {
    ...
    const reasoning: ReasoningStep[] = [
      ...
      { label: 'Why a human decides', detail: 'Refinancing involves lock-in, legal subsidy clawback and TDSR checks — a person should walk you through it.' },
    ];
    ...
    return {
      ...
      summary: `Your income rose and 2-year fixed rates fell to ~${pct(p.newRate)}. Refinancing your ${money(p.outstanding)} loan from ${pct(p.currentRate)} could save about ${money(p.monthlySaving)}/month. This is a big, mostly irreversible decision — I've prepared a full pack and lined up a human RM.`,
      reasoning,
      confidence: 0.78,
